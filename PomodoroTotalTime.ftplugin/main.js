define(function(require, exports, module) {
  ...
  Extensions.addInit(function (editor) {
    editor.addKeyMap({
      ...
      'Cmd-K' : function (editor) {
        var pomodoros    = 0;
        var totalMinutes = 0;

        var nodeHasTag = function(node, tag) {
          return (eval('node.tags().' + tag) != undefined)
        };

        tree.nodes().forEach(function(node) {
          if (nodeHasTag(node, 'p') == true) {
            pomodoros = tags.p.match(/x/g).length;
          }
          //
          // if nodeHasTag(node, 'pt') {
          //   var minutesArray = tags.pt.split(',');
          //   totalMinutes = eval(minutesArray.join('+'))
          // }
        });
      }
    });
